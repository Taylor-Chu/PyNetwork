...
model_gpu.add( PyNetwork.layers.Dense_GPU(hidden_nodes=100, activation_function='relu', l2=0.0, l1=0.0,context=context, queue=queue, gpuoperator=gpuoperator) )
...
optimizer = PyNetwork.optimizers.RMSprop_GPU(gpuoperator=gpuoperator,learning_rate=0.0005)
model_gpu.build(loss_function='mse', optimizer=optimizer, metrics='accuracy')

model_gpu.train(x_train_gpu, y_train_gpu, epochs=10, batch_size=128, verbose=True)
print(model_gpu.evaluate(x_test_gpu, y_test_gpu))