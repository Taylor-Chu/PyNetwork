...
from PyNetwork import get_error_function_gpu, get_metric_function_gpu
from PyNetwork.gpu.GPUNN import GPUOPERATOR
from PyNetwork.optimizers import Optimizer, get_optimizer
from PyNetwork.layers import Flatten_GPU, Input_GPU, BatchNorm_GPU
...
class Sequential_GPU:
    ...
    def predict(self, x_train, output_only=True):
        """ Forward propagate the input through the layers
            ...
            x_train : np.array
                ...
                x_train is assmed to be on GPU
            ...
            dict of int - cl.array, dict of int - cl.array
    ...
    def evaluate(self, x_test, y_test):
        """ Return the MSE of the model prediction
            ...
            y_test : np.array
                ...
                x_test and y_test are assumed to be on GPU
            Returns
            -------
            str
                The error
    ...
    def train(self, x_train, y_train, epochs=100, batch_size=None, verbose=True):
        """ Train the neural network by means of back propagation
            ...
            x_train : np.array
                ...
                x_train is assumed to be on GPU
            y_train : np.array
                ...
                y_train is assumed to be on GPU
    ...
    @property
    def loss(self):
        # Use GPU calculate error function
        return get_error_function_gpu(self.loss_function, self.gpuoperator)

    @property
    def metric(self):
        if self.metric_function is not None:
            return get_metric_function_gpu(self.metric_function)
        return None
    ...
    def _back_prop(self, x_train, y_train):
        """ Perform a single back propagation of the batch `x_train`, `y_train`.
            ...
            x_train and y_train are assumed to be on GPU
        """
        ...
        for i in range(self.n, 2, -1):  # i = 1 is the Input class, no backpropagation needed
            ...
            if isinstance(self.layers[i], BatchNorm_GPU):
