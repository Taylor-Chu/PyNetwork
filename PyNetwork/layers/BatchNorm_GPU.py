...
class BatchNormGrads:
    ...
    def dsigma2(gpuoperator,z, dz_hat_, epsilon, mu=None, sigma=None):
        ...
        half = cl_array.to_device(gpuoperator.queue,np.array([-0.5]))
        c = gpuoperator.pow(gpuoperator.mul(half,gpuoperator.add(gpuoperator.pow(sigma,2),epsilon)),-1.5)
        #return c * np.sum(dz_hat_ * (z - mu), axis=0)
        return gpuoperator.mul(c,gpuoperator.sum(gpuoperator.mul(gpuoperator.sub(z,mu),dz_hat_),axis=0))
    @staticmethod
    def dmu(gpuoperator,z, dz_hat_, epsilon, mu=None, sigma=None, dsigma2_=None):
        ...
        m_one = cl_array.to_device(gpuoperator.queue,np.array([-1]))
        m_two = cl_array.to_device(gpuoperator.queue,np.array([-2]))
        #return (-1 / np.sqrt(sigma**2 + epsilon))*np.sum(dz_hat_, axis=0) + dsigma2_ * np.mean(-2*(z - mu), axis=0)
        return gpuoperator.add(gpuoperator.mul(gpuoperator.div(m_one,
                                                               gpuoperator.sqrt(gpuoperator.add(gpuoperator.pow(sigma,2),
                                                                                                epsilon))),
                                               gpuoperator.sum(dz_hat_,axis=0)),
                               gpuoperator.mul(dsigma2_,
                                               gpuoperator.mean(gpuoperator.mul(m_two,gpuoperator.sub(z,mu))),axis=0))
    @staticmethod
    def dz(gpuoperator,z, new_delta, gamma, epsilon, mu=None, sigma=None):
        ...
        if sigma is None:
            sigma = gpuoperator.std(z, axis=0)
        m = cl_array.to_device(gpuoperator.queue,np.array([len(z.get())]))
        two= cl_array.to_device(gpuoperator.queue,np.array([2]))
        ...
        return gpuoperator.add(gpuoperator.div(dz_hat_,
                                               gpuoperator.sqrt(gpuoperator.add(gpuoperator.pow(sigma,2),epsilon))),
                               gpuoperator.add(gpuoperator.mul(gpuoperator.mul(dsigma2_,two),
                                                               gpuoperator.div(gpuoperator.sub(z,mu),
                                                                               m)),
                                               gpuoperator.div(dmu_,m)))
...
class BatchNorm(Layer):
    ...
    def build(self, previous_output_shape):
        ...
        self.epsilon_gpu=cl_array.to_device(self.queue,self.epsilon)
    ...
    def predict(self, z, output_only=True, **kwargs):
        ...
        b = self.gpuoperator.div(self.gpuoperator.sub(z,mean),self.gpuoperator.sqrt(self.gpuoperator.add(self.gpuoperator.pow(std,2),self.epsilon_gpu)))
    ...
    def get_weight_grad_(self, delta, prev_z):
        ...
        z_hat = self.gpuoperator.div(self.gpuoperator.sub(prev_z,self.gpuoperator.mean(prev_z,axis=0)),
                                     self.gpuoperator.sqrt(self.gpuoperator.add(self.gpuoperator.pow(self.gpuoperator.std(prev_z,axis=0),2),
                                                                                self.epsilon_gpu)))
